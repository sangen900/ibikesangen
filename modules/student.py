...
# Callback function that first checks availability and assigns a student to a group if it is not already full.
# if a student is the 5th member of a group, that group will be removed from the available, displayed groups for all other players
...
def display_group_info():
	...
	st.write(f"Group Member Count: {group_state['player_count']}")
...
def init():
	game_state = game.load()
	if len(game_state['available_groups']) > 0:
		if not ss.name:
			st.title('Welcome to the Student Page!')
...
# Function to be called on each student page to display widgets to select orders and pass
...
def display_current_orders():
	...
	if len(ss.group_state['orders']) > 0:
		...
		if len(order_keys) > 0:
			...
			with col2:
				st.write('please select an order to contribute to')
