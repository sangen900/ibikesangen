...
with tab2:
    ...
    if fb_d_m != "":
        with open("fb_d_m.txt", "w") as f:
            f.write(fb_d_m)
        st.markdown("---")

    if st.button('Clear Feedback', key=0):
        if path.isfile('fb_d_m.txt'):
            os.remove('fb_d_m.txt')


    st.markdown("---")
    text = ""
    if path.isfile('fb_d_i.txt'):
        with open('fb_d_i.txt', 'r') as f:
            text = f.read()

    fb_d_i = st.text_area("Your feedback to the Industrial Engineer", text)
    if fb_d_i != "":
        with open("fb_d_i.txt", "w") as f:
            f.write(fb_d_i)
        st.markdown("---")

    if st.button('Clear Feedback', key=1):
        if path.isfile('fb_d_i.txt'):
            os.remove('fb_d_i.txt')

    
    st.markdown("---")
    text = ""
    if path.isfile('fb_d_pm.txt'):
        with open('fb_d_pm.txt', 'r') as f:
            text = f.read()

    fb_d_pm = st.text_area("Your feedback to the Project Manager", text)
    if fb_d_pm != "":
        with open("fb_d_pm.txt", "w") as f:
            f.write(fb_d_pm)
        st.markdown("---")

    if st.button('Clear Feedback', key=2):
        if path.isfile('fb_d_pm.txt'):
            os.remove('fb_d_pm.txt')
    ...
    if fb_d_pum != "":
        with open("fb_d_pum.txt", "w") as f:
            f.write(fb_d_pum)
        st.markdown("---")

    if st.button('Clear Feedback', key=3):
        if path.isfile('fb_d_pum.txt'):
            os.remove('fb_d_pum.txt')
    ...
    if path.isfile('fb_i_d.txt'):
        st.markdown("---")
        st.write("Feedback from the **:red[Industrial Engineer]**")
        with open('fb_i_d.txt', 'r') as f:
            text = f.read()
        st.write(text)
        st.markdown("---")
