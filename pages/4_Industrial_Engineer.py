...
with tab2:
    ...
    if fb_i_d != "":
        with open("fb_i_d.txt", "w") as f:
            f.write(fb_i_d)
        st.markdown("---")

    if st.button('Clear Feedback', key=0):
        if path.isfile('fb_i_d.txt'):
            os.remove('fb_i_d.txt')
    ...
    if fb_i_m != "":
        with open("fb_i_m.txt", "w") as f:
            f.write(fb_i_m)
        st.markdown("---")

    if st.button('Clear Feedback', key=1):
        if path.isfile('fb_i_m.txt'):
            os.remove('fb_i_m.txt')

    
    st.markdown("---")
    text = ""
    if path.isfile('fb_i_pm.txt'):
        with open('fb_i_pm.txt', 'r') as f:
            text = f.read()

    fb_i_pm = st.text_area("Your feedback to the Project Manager", text)
    if fb_i_pm != "":
        with open("fb_i_pm.txt", "w") as f:
            f.write(fb_i_pm)
        st.markdown("---")

    if st.button('Clear Feedback', key=2):
        if path.isfile('fb_i_pm.txt'):
            os.remove('fb_i_pm.txt')
    ...
    if fb_i_pum != "":
        with open("fb_i_pum.txt", "w") as f:
            f.write(fb_i_pum)
        st.markdown("---")

    if st.button('Clear Feedback', key=3):
        if path.isfile('fb_i_pum.txt'):
            os.remove('fb_i_pum.txt')
