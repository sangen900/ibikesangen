...
with tab2:
    ...
    st.markdown("---")
    text = ""
    if path.isfile('fb_m_d.txt'):
        with open('fb_m_d.txt', 'r') as f:
            text = f.read()

    fb_m_d = st.text_area("Your feedback to the Design Engineer", text)
    if fb_m_d != "":
        with open("fb_m_d.txt", "w") as f:
            f.write(fb_m_d)
        st.markdown("---")

    if st.button('Clear Feedback', key=0):
        if path.isfile('fb_m_d.txt'):
            os.remove('fb_m_d.txt')


    st.markdown("---")
    text = ""
    if path.isfile('fb_m_i.txt'):
        with open('fb_m_i.txt', 'r') as f:
            text = f.read()

    fb_m_i = st.text_area("Your feedback to the Industrial Engineer", text)
    if fb_m_i != "":
        with open("fb_m_i.txt", "w") as f:
            f.write(fb_m_i)
        st.markdown("---")

    if st.button('Clear Feedback', key=1):
        if path.isfile('fb_m_i.txt'):
            os.remove('fb_m_i.txt')

    
    st.markdown("---")
    text = ""
    if path.isfile('fb_m_pm.txt'):
        with open('fb_m_pm.txt', 'r') as f:
            text = f.read()

    fb_m_pm = st.text_area("Your feedback to the Project Manager", text)
    if fb_m_pm != "":
        with open("fb_m_pm.txt", "w") as f:
            f.write(fb_m_pm)
        st.markdown("---")

    if st.button('Clear Feedback', key=2):
        if path.isfile('fb_m_pm.txt'):
            os.remove('fb_m_pm.txt')

    st.markdown("---")
    text = ""
    if path.isfile('fb_m_pum.txt'):
        with open('fb_m_pum.txt', 'r') as f:
            text = f.read()

    fb_m_pum = st.text_area("Your feedback to the Purchasing Manager", text)
    if fb_m_pum != "":
        with open("fb_m_pum.txt", "w") as f:
            f.write(fb_m_pum)
        st.markdown("---")

    if st.button('Clear Feedback', key=3):
        if path.isfile('fb_m_pum.txt'):
            os.remove('fb_m_pum.txt')


    # reading
    st.header("Feedback **:red[From]**")
    if path.isfile('fb_d_m.txt'):
        st.markdown("---")
        st.write("Feedback from the **:red[Design Engineer]**")
        with open('fb_d_m.txt', 'r') as f:
            text = f.read()
        st.write(text)
        st.markdown("---")

    if path.isfile('fb_i_m.txt'):
        st.markdown("---")
        st.write("Feedback from the **:red[Industrial Engineer]**")
        with open('fb_i_m.txt', 'r') as f:
            text = f.read()
        st.write(text)
        st.markdown("---")
